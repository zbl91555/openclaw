 ...
 *
  * Supports both legacy card format and new raw_card_content format
  * (json_card + json_attachment + card_schema).
 */
...
function normalizeTimeFormat(input) {
    ...
    // "2024-11-26 14:00:00" or "2024-11-26 14:00"
    ...
    // Date-only "2024-11-26" — keep as is
    if (/^\d{4}-\d{2}-\d{2}$/.test(input))
        return input;
    // Time-only "14:00:00" or "14:00" — keep as is
    if (/^\d{2}:\d{2}(:\d{2})?$/.test(input))
        return input;
    return input;
}
// ============================================================================
// CardConverter class (converter.go)
// ============================================================================
var Mode;
(function (Mode) {
...
class CardConverter {
    ...
    extractHeaderTitle(header, _schema) {
        // Schema 1: header.property.title
    ...
    convertBody(body, _schema) {
        let elements;
        // Card Schema 2: body.property.elements
        ...
        // Fallback: body.elements (Schema 1)
    ...
    // ---- Text elements (elements_text.go) ----
    ...
    convertCodeBlock(prop) {
        let language = prop.language || "plaintext";
        let code = "";
    ...
    // ---- Layout elements (elements_layout.go) ----
    ...
    // ---- Action elements (elements_action.go) ----
    ...
    // ---- Person elements (elements_person.go) ----
    ...
    // ---- Media elements (elements_media.go) ----
    ...
    convertChart(prop, _id) {
        ...
        if (chartSpec && typeof chartSpec === "object") {
            ...
            if (ct) {
                chartType = ct;
                const typeName = chartTypeNames[chartType];
                if (typeName)
    ...
    extractTextStyle(prop) {
        const style = { bold: false, italic: false, strikethrough: false };
...
// ============================================================================
// Legacy card converter (preserves old behavior for non-raw_card_content)
// ============================================================================
...
// ============================================================================
// Entry point (ContentConverterFn)
// ============================================================================
export const convertInteractive = (raw) => {
    ...
    // New format: has json_card field
