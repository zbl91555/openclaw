...
async function diagnoseAccount(account) {
    ...
    try {
        ...
        checks.push({
            ...
            details: scopesResult.pending > 0
                ? "存在未授权的权限，可能影响部分功能"
                : undefined,
        });
    }
...
export async function runDiagnosis(params) {
    ...
    // -- Account count --
    const accountIds = getLarkAccountIds(config);
    globalChecks.push({
        name: "飞书账户数量",
    ...
    // -- Tools --
    const tools = detectRegisteredTools(config);
    // -- Recent errors --
 ...
 */
export function analyzeTrace(lines, messageId) {
