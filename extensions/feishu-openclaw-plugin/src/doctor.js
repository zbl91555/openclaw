 ...
 * 检查基础信息和账号状态
 */
async function checkBasicInfo(account) {
    const lines = [];
    let apiOk = false;
 ...
 * 生成诊断摘要，只显示问题和基本描述
 */
function generateSummary(result, overallStatus) {
    const lines = [];
    const issues = [];
    // 检查各种问题
    if (!result.basicInfoOk) {
        issues.push("❌ API 连接失败 - 无法连接到飞书开放平台");
    }
    if (result.appStatus === "fail" && result.appMissingScopes && result.appMissingScopes.length > 0) {
        issues.push(`❌ 应用权限不足 - 缺少 ${result.appMissingScopes.length} 个必需权限`);
    }
    if (!result.userHasAuth) {
        issues.push("⚠️ 用户未授权 - 需要完成 OAuth 授权流程");
    }
    else {
        if (result.userTokenExpired) {
            issues.push("⚠️ 用户授权已过期 - Token 需要刷新");
        }
        if (result.userMissingScopes && result.userMissingScopes.length > 0) {
            issues.push(`⚠️ 用户权限不足 - 缺少 ${result.userMissingScopes.length} 个权限`);
        }
    }
    // 生成摘要
    lines.push("## 📋 诊断摘要");
...
export async function runFeishuDoctor(config) {
    ...
    // 2. 检查基础信息和账号状态
    const basicInfoResult = await checkBasicInfo(account);
    // 3. 检查应用权限
    const appResult = await checkAppPermissions(account, sdk);
    // 4. 检查用户权限
    const userResult = await checkUserPermissions(account, sdk);
    // 5. 计算总体状态
    const overallStatus = calculateOverallStatus(appResult.status, userResult.status);
    // 6. 生成报告头部
    lines.push("# 飞书插件诊断");
    lines.push("");
    lines.push(`插件版本: ${PLUGIN_VERSION}  |  诊断时间: ${formatTimestamp(new Date())}`);
    lines.push("");
    lines.push("---");
    lines.push("");
    // // 7. 生成诊断摘要（包含总体状态）
