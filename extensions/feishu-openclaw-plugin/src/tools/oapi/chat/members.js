...
const ChatMembersSchema = Type.Object({
    chat_id: Type.String({
        description: "群 ID（格式如 oc_xxx）。" +
            "可以通过 feishu_chat_search 工具搜索获取",
    }),
    member_id_type: Type.Optional(Type.Union([
    ...
    api.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                assertLarkOk(res);
                const memberCount = res.data?.items?.length ?? 0;
                const memberTotal = res.data?.member_total ?? 0;
