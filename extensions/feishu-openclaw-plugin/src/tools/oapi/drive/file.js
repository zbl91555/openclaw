...
// 分片上传配置
const CHUNK_SIZE = 4 * 1024 * 1024; // 4MB，飞书固定分片大小
...
const FeishuDriveFileSchema = Type.Union([
    ...
    Type.Object({
        ...
        order_by: Type.Optional(Type.Union([
            Type.Literal("EditedTime"),
    ...
    Type.Object({
        ...
        parent_type: Type.Union([
            ...
            description: "父节点类型：explorer（云空间）、bitable（多维表格）",
        }),
        parent_node: Type.Optional(Type.String({
...
export function registerFeishuDriveFileTool(api) {
    ...
    api.registerTool({
        ...
        description: "【以用户身份】飞书云空间文件管理工具。当用户要求查看文件列表、获取文件信息、复制/移动/删除文件、上传/下载文件时使用。" +
            "\n\nActions:" +
                ...
                switch (p.action) {
                    ...
                    case "get_meta": {
                        if (!p.request_docs || !Array.isArray(p.request_docs) || p.request_docs.length === 0) {
                            return json({
                                error: "参数错误：request_docs 必须是非空数组。正确格式：{action: 'get_meta', request_docs: [{doc_token: '...', doc_type: 'sheet'}]}",
                            });
                        }
                    ...
                    case "copy": {
                        ...
                        assertLarkOk(res);
                        log.info(`copy: new file_token=${res.data?.file?.token ?? 'unknown'}`);
                        return json({
                            file: res.data?.file,
                        });
                    }
                    ...
                    case "move": {
                        ...
                        assertLarkOk(res);
                        log.info(`move: task_id=${res.data?.task_id}`);
                        return json({
                            success: true,
                    ...
                    case "delete": {
                        ...
                        assertLarkOk(res);
                        log.info(`delete: task_id=${res.data?.task_id}`);
                        return json({
                            success: true,
                            task_id: res.data?.task_id,
                            file_token: p.file_token,
                    ...
                    case "upload": {
                        ...
                        if (p.file_path) {
                            ...
                            try {
                                ...
                                return json({
                                    error: `读取本地文件失败: ${err instanceof Error ? err.message : String(err)}`,
                                });
                            }
                        }
                            ...
                            if (!p.file_name || !p.size) {
                                return json({
                                    error: "使用 file_content_base64 时，必须提供 file_name 和 size 参数",
                                });
                            }
                            ...
                            return json({
                                error: "必须提供 file_path 或 file_content_base64 参数之一",
                            });
                        }
                        // 根据文件大小选择上传方式
                        if (fileSize <= SMALL_FILE_THRESHOLD) {
                            ...
                            if (!prepareRes) {
                                return json({ error: "预上传失败：返回结果为空" });
                            }
