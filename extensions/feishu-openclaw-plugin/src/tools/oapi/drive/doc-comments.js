...
const DocCommentsSchema = Type.Object({
    ...
    elements: Type.Optional(Type.Array(ReplyElementSchema, {
        description: "评论内容元素数组(action=create时必填)。" +
        ...
        if (comment.reply_list?.replies?.length > 0 || comment.has_more) {
            try {
                ...
                while (hasMore) {
                    ...
                    if (replyRes.code === 0 && replyRes.data?.items) {
                        replies.push(...(replyRes.data.items || []));
                        hasMore = replyRes.data.has_more || false;
                        pageToken = replyRes.data.page_token;
                    }
                    else {
                        break;
                    }
                ...
                if (p.file_type === "wiki") {
                    ...
                    try {
                        ...
                        if (!node || !node.obj_token || !node.obj_type) {
                            return json({
                                error: `无法解析 wiki token "${p.file_token}" 到实际文档对象（可能是目录节点而非文档）`,
                                wiki_node: node,
                            });
                        }
                        ...
                        return json({
                            error: `无法解析 wiki token "${p.file_token}": ${err}`,
                        });
                    }
                }
                // Action: list - 获取评论列表
