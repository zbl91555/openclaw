            ...
            try {
                ...
                switch (p.action) {
                    ...
                    case "create": {
                        ...
                        if (p.records) {
                            return json({
                                error: "参数错误：create action 不接受 'records' 参数",
                                hint: "create 用于创建单条记录，使用 'fields' 参数。如需批量创建，请使用 action: 'batch_create' + 'records' 参数。",
                                correct_format: {
                                    action: "create",
                                    fields: { "字段名": "字段值" },
                                },
                        ...
                        if (!p.fields || Object.keys(p.fields).length === 0) {
                            return json({
                                error: "参数错误：fields 参数不能为空",
                                hint: "create action 需要 'fields' 参数，格式：{ '字段名': '字段值', ... }",
                            });
                        }
                    ...
                    case "update": {
                        ...
                        if (p.records) {
                            return json({
                                error: "参数错误：update action 不接受 'records' 参数",
                                hint: "update 用于更新单条记录，使用 'record_id' + 'fields' 参数。如需批量更新，请使用 action: 'batch_update' + 'records' 参数。",
                                correct_format: {
                                    action: "update",
                                    record_id: "recXXX",
                                    fields: { "字段名": "字段值" },
                    ...
                    case "batch_create": {
                        ...
                        if (p.fields) {
                            return json({
                                error: "参数错误：batch_create action 不接受 'fields' 参数",
                                hint: "batch_create 用于批量创建记录，使用 'records' 参数（数组）。如只需创建单条记录，请使用 action: 'create' + 'fields' 参数。",
                                correct_format: {
                                    action: "batch_create",
                                    records: [{ fields: { "字段名": "字段值" } }],
                                },
                        ...
                        if (!p.records || p.records.length === 0) {
                            return json({
                                error: "参数错误：records 参数不能为空",
                                hint: "batch_create 需要 'records' 数组参数，格式：[{ fields: {...} }, ...]",
                            });
                        }
                        if (p.records.length > 500) {
                            return json({
                                error: "参数错误：records 数量超过限制（最多 500 条）",
                    ...
                    case "batch_update": {
                        ...
                        if (p.record_id || p.fields) {
                            return json({
                                error: "参数错误：batch_update action 不接受 'record_id' 或 'fields' 参数",
                                hint: "batch_update 用于批量更新记录，使用 'records' 数组参数。如只需更新单条记录，请使用 action: 'update' + 'record_id' + 'fields' 参数。",
                                correct_format: {
                                    action: "batch_update",
                                    records: [
                                        { record_id: "recXXX", fields: { "字段名": "字段值" } },
                        ...
                        if (!p.records || p.records.length === 0) {
                            return json({
                                error: "参数错误：records 参数不能为空",
                                hint: "batch_update 需要 'records' 数组参数，格式：[{ record_id: 'recXXX', fields: {...} }, ...]",
                            });
                        }
                        if (p.records.length > 500) {
                            return json({
                    ...
                    case "list": {
                        ...
                        assertLarkOk(res);
                        log.info(`list: returned ${res.data?.items?.length ?? 0} records`);
                        return json({
