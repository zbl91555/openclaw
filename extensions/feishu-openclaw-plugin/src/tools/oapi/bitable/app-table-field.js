            ...
            try {
                ...
                switch (p.action) {
                    ...
                    case "create": {
                        ...
                        assertLarkOk(res);
                        log.info(`create: created field ${res.data?.field?.field_id ?? 'unknown'}`);
                        return json({
                            field: res.data?.field ?? res.data,
                        });
                    }
                    // -----------------------------------------------------------------
                    // LIST
                    ...
                    case "list": {
                        ...
                        assertLarkOk(res);
                        log.info(`list: returned ${res.data?.items?.length ?? 0} fields`);
                        return json({
                    ...
                    case "update": {
                        ...
                        if (!finalType || !finalFieldName) {
                            ...
                            if (!currentField) {
                                return json({
                                    error: `字段 ${p.field_id} 不存在`,
                                    hint: "请检查 field_id 是否正确，可使用 list action 查看所有字段。",
                                });
                            }
                            // 合并：用户传的优先，否则用查询到的
                            ...
                            finalProperty =
                                p.property !== undefined ? p.property : currentField.property;
