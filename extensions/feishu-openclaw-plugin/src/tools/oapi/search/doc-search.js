...
import { json, createToolContext, handleInvokeErrorWithAutoAuth, convertTimeRange, } from "../helpers.js";
// ---------------------------------------------------------------------------
// Schema - Helper types
// ---------------------------------------------------------------------------
...
function unixTimestampToISO8601(raw) {
    ...
    const shanghaiDate = new Date(utcMs + SHANGHAI_UTC_OFFSET_HOURS * 60 * 60 * 1000);
    if (Number.isNaN(shanghaiDate.getTime()))
        return null;
    const year = shanghaiDate.getUTCFullYear();
    const month = pad2(shanghaiDate.getUTCMonth() + 1);
    const day = pad2(shanghaiDate.getUTCDate());
    const hour = pad2(shanghaiDate.getUTCHours());
    const minute = pad2(shanghaiDate.getUTCMinutes());
    const second = pad2(shanghaiDate.getUTCSeconds());
