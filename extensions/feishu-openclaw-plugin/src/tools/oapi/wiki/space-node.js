...
const FeishuWikiSpaceNodeSchema = Type.Union([
    ...
    Type.Object({
        ...
        node_type: Type.Optional(Type.Union([
            Type.Literal("origin"),
...
export function registerFeishuWikiSpaceNodeTool(api) {
    ...
    api.registerTool({
        ...
        description: "Feishu Wiki Space Node management tool. Actions: list, get, create, move, copy. " +
            "Node is a document in wiki space, including doc, sheet, mindnote, etc. " +
            "node_token is the unique identifier of a node, obj_token is the actual document token.",
            ...
            try {
                ...
                switch (p.action) {
                    ...
                    case "list": {
                        ...
                        assertLarkOk(res);
                        log.info(`list: returned ${res.data?.items?.length ?? 0} nodes`);
                        return json({
