...
import { assertLarkOk, createToolContext, getFirstAccount, handleInvokeErrorWithAutoAuth, json, } from "../helpers.js";
import { dateTimeToSecondsString, parseTimeRangeToSeconds } from "./time-utils.js";
import { formatMessageList } from "./format-messages.js";
import { getUATUserName, batchResolveUserNamesAsUser, } from "./user-name-uat.js";
// ===========================================================================
// Shared helpers
// ===========================================================================
...
async function resolveP2PChatId(client, openId, log) {
    ...
    if (!chats?.length) {
        ...
        throw new Error(`未找到与 open_id=${openId} 的单聊会话。可能你和该用户之间没有单聊记录。`);
    }
...
const GetMessagesSchema = Type.Object({
    ...
    sort_rule: Type.Optional(Type.Union([Type.Literal("create_time_asc"), Type.Literal("create_time_desc")], { description: "排序方式，默认 create_time_desc（最新消息在前）" })),
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                if (p.open_id && p.chat_id) {
                    return json({ error: "open_id 和 chat_id 不能同时提供，请只传其中一个" });
                }
                if (!p.open_id && !p.chat_id) {
                    return json({ error: "open_id 和 chat_id 必须提供其中一个" });
                }
                if (p.relative_time && (p.start_time || p.end_time)) {
                    return json({ error: "relative_time 和 start_time/end_time 不能同时使用" });
                }
...
const SearchMessagesSchema = Type.Object({
    ...
    message_type: Type.Optional(Type.Union([Type.Literal("file"), Type.Literal("image"), Type.Literal("media")], { description: "消息类型过滤：file / image / media。为空则搜索所有类型" })),
    sender_type: Type.Optional(Type.Union([Type.Literal("user"), Type.Literal("bot"), Type.Literal("all")], { description: "发送者类型：user / bot / all。默认 user" })),
...
async function resolveP2PTargetNames(chatMap, client, logFn) {
    const ids = [
        ...new Set([...chatMap.values()].map((c) => c.p2p_target_id).filter((id) => !!id)),
    ];
    if (ids.length > 0) {
