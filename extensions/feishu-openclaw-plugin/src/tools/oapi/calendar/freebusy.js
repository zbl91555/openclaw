...
import { json, createToolContext, parseTimeToRFC3339, assertLarkOk, handleInvokeErrorWithAutoAuth, } from "../helpers.js";
// ---------------------------------------------------------------------------
// Schema
// ---------------------------------------------------------------------------
        ...
        async execute(_toolCallId, params) {
            ...
            try {
                ...
                if (!p.user_ids || p.user_ids.length === 0) {
                    ...
                    return json({
                        error: "user_ids 不能为空，必须提供 1-10 个用户 ID",
                    });
                }
                if (p.user_ids.length > 10) {
                    log.warn(`[FREEBUSY] user_ids exceeds limit: ${p.user_ids.length}`);
                    return json({
                        error: `user_ids 数量超过限制，最多支持 10 个用户（当前: ${p.user_ids.length}）`,
                    });
                }
                ...
                if (!timeMin || !timeMax) {
                    ...
                    return json({
                        error: "时间格式错误！必须使用ISO 8601 / RFC 3339 格式（包含时区），例如 '2024-01-01T00:00:00+08:00'，例如 '2026-02-25 14:00:00'。",
